 ...
 *
 * Usage:
 *   npx @blockrun/IgniteRouter              # Start standalone proxy
 *   npx @blockrun/IgniteRouter --version    # Show version
 *   npx @blockrun/IgniteRouter --port 8402  # Custom port
 *
 * For production deployments, use with PM2:
 *   pm2 start "npx @blockrun/IgniteRouter" --name IgniteRouter
 */
...
Usage:
  ...
  IgniteRouter status                    # Live proxy status (wallet, balance, chain)
  IgniteRouter wallet                    # Wallet details + balance
...
Management Commands:
  ...
  wallet recover    Restore wallet.key from mnemonic (if generated by IgniteRouter)
...
For more info: https://github.com/BlockRunAI/IgniteRouter
`);
}
...
async function cmdStatus(port: number): Promise<void> {
  try {
    ...
    console.error(`  Is the proxy running? Start with: npx @blockrun/IgniteRouter`);
    process.exit(1);
  }
}
...
async function main(): Promise<void> {
  ...
  if (args.chain) {
    ...
    if (targetChain === "solana") {
      ...
      if (existsSync(MNEMONIC_FILE)) {
        ...
        console.log(`[IgniteRouter] Solana wallet already set up.`);
      } else {
        // First time — generate mnemonic + keypair
        console.log(`[IgniteRouter] Setting up Solana wallet...`);
        const { solanaPrivateKeyBytes } = await setupSolana();
        solanaAddr = await getSolanaAddress(solanaPrivateKeyBytes);
        console.log(`[IgniteRouter] Solana wallet created.`);
        console.log(`[IgniteRouter] ⚠  Back up your mnemonic: ${MNEMONIC_FILE}`);
      }

      await savePaymentChain("solana");
      console.log(`[IgniteRouter] Payment chain → Solana`);
      console.log(`[IgniteRouter] Solana address : ${solanaAddr}`);
      console.log(`[IgniteRouter] Fund USDC at   : https://solscan.io/account/${solanaAddr}`);
    } else {
      await savePaymentChain("base");
      console.log(`[IgniteRouter] Payment chain → Base (EVM)`);
    }

    console.log(
      `[IgniteRouter] Restart gateway to apply: systemctl --user restart openclaw-gateway.service`,
    );
    console.log(`[IgniteRouter]   or: openclaw gateway restart`);
    process.exit(0);
  }
  ...
  if (wallet.source === "generated") {
    console.log(`[IgniteRouter] Generated new wallet: ${wallet.address}`);
  } else if (wallet.source === "saved") {
    console.log(`[IgniteRouter] Using saved wallet: ${wallet.address}`);
  } else if (wallet.source === "config") {
    console.log(`[IgniteRouter] Using wallet from plugin config: ${wallet.address}`);
  } else {
    console.log(`[IgniteRouter] Using wallet from BLOCKRUN_WALLET_KEY: ${wallet.address}`);
  }
  ...
  if (wallet.solanaPrivateKeyBytes) {
    try {
      ...
      console.log(`[IgniteRouter] Solana address: ${solAddr}`);
    } catch {
      // Non-fatal
    }
  }
  ...
  const proxy = await startProxy({
    ...
    onRouted: (decision) => {
      ...
      console.log(`[IgniteRouter] [${decision.tier}] ${decision.model} $${cost} (saved ${saved}%)`);
    },
    onLowBalance: (info) => {
      console.warn(`[IgniteRouter] Low balance: ${info.balanceUSD}. Fund: ${info.walletAddress}`);
    },
    onInsufficientFunds: (info) => {
      console.error(
        `[IgniteRouter] Insufficient funds. Balance: ${info.balanceUSD}, Need: ${info.requiredUSD}`,
      );
  ...
  try {
    ...
    if (balance.isEmpty) {
      console.log(`[IgniteRouter] Wallet balance: $0.00 (using FREE model)`);
      console.log(`[IgniteRouter] Fund wallet for premium models: ${displayAddress}`);
    } else if (balance.isLow) {
      console.log(`[IgniteRouter] Wallet balance: ${balance.balanceUSD} (low)`);
    } else {
      console.log(`[IgniteRouter] Wallet balance: ${balance.balanceUSD}`);
    }
    ...
    console.log(`[IgniteRouter] Wallet: ${displayAddress} (balance check pending)`);
  }
