...
async function fetchLatestVersion(): Promise<string | null> {
  try {
    const res = await fetch("https://registry.npmjs.org/@blockrun/IgniteRouter/latest", {
...
function identifyIssues(result: DiagnosticResult): string[] {
  ...
  if (result.latestVersion && result.latestVersion !== result.version) {
    issues.push(
      `Outdated version: running v${result.version}, latest is v${result.latestVersion}. Run: curl -fsSL https://blockrun.ai/IgniteRouter-update | bash`,
    );
  }
...
function printDiagnostics(result: DiagnosticResult): void {
  ...
  if (result.latestVersion && result.latestVersion !== result.version) {
    ...
    console.log(
      `  ${yellow(`Update:    curl -fsSL https://blockrun.ai/IgniteRouter-update | bash`)}`,
    );
    ...
    const response = await paymentFetch("https://blockrun.ai/api/v1/chat/completions", {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: "system",
            content: `You are a technical support expert for BlockRun and IgniteRouter.
